...
export default function PoliciesPage() {
  const policies = [
    ...
    },
    {
      ...
      actions: ["agent:read", "skill:invoke", "task:create"],
      priority: 50,
    },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Authorization • RBAC/ABAC • Least Privilege • Delegation Chain
      </p>
        ...
        {policies.map(p => (
          <Card key={p.id}>
            ...
            <CardContent className="text-xs space-y-1">
              ...
              <div>
                Resources: {p.resources?.join(", ") ?? (p as any).resources?.join(", ") ?? "any"}
              </div>
              <div>Actions: {(p as any).actions?.join(", ") ?? "any"}</div>
              {(p as any).conditions && (
                <div>Conditions: {JSON.stringify((p as any).conditions)}</div>
              )}
            </CardContent>
          </Card>
        ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Delegation Example</CardTitle>
        </CardHeader>
        <CardContent>
