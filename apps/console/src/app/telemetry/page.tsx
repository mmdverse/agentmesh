  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        OpenTelemetry • Trace: User → Agent A → Agent B → Tool • Metrics: latency, errors, retries
      </p>

      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Recent Spans ({spans.total})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 max-h-96 overflow-auto">
            {spans.spans?.slice(-20).map((span: any) => (
              <div key={span.id} className="border rounded p-2 text-xs">
                <div className="font-medium">
                  {span.name} • {span.status} • {span.durationMs}ms
                </div>
                <div className="text-muted-foreground">
                  Trace: {span.traceId.slice(0, 12)} • ID: {span.id.slice(0, 12)} • Parent:{" "}
                  {span.parentSpanId?.slice(0, 12) ?? "root"}
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Metrics ({metrics.total})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 max-h-96 overflow-auto">
            {metrics.metrics?.slice(-20).map((m: any, i: number) => (
              <div key={i} className="border rounded p-2 text-xs">
                ...
                <div className="text-muted-foreground">
                  Labels: {JSON.stringify(m.labels)} • {new Date(m.timestamp).toLocaleTimeString()}
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Trace Propagation</CardTitle>
        </CardHeader>
        <CardContent>
