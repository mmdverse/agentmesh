import { getMCPBridge, InMemoryMCPClient, type MCPServerInfo, type MCPTool, ExampleMCPServer } from "@agentmesh/mcp-bridge";
...
export class MCPService {
  ...
  async registerServer(input: { name: string; endpoint: string; serverInfo?: MCPServerInfo; tools?: MCPTool[]; tenant?: TenantContext }): Promise<{ name: string; card: any }> {
    ...
    for (const tool of serverInfo.tools) {
      client.registerToolHandler(tool.name, async (args) => {
        return { result: `Executed ${tool.name} with args ${JSON.stringify(args)}`, server: input.name, timestamp: new Date().toISOString() };
      });
    }
  ...
  async callTool(serverName: string, toolName: string, args: Record<string, unknown>, tenant?: TenantContext): Promise<{ output: unknown; isError?: boolean }> {
    // In this direction, A2A agent calls MCP tool via bridge
    const result = await this.bridge.a2aToMCP(serverName, toolName, { message: JSON.stringify(args), context: args });
    return result;
  }

  async callA2AFromMCP(mcpToolName: string, args: Record<string, unknown>, a2aCaller: (agentId: string, skillId: string, message: string) => Promise<unknown>): Promise<unknown> {
